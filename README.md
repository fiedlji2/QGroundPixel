...
- **Decoder chosen per codec** (WFB‑NG settings): H.264 sources such as the XFRobot Z2 use
  ...
  codec applies the matching decoder automatically — including rebuilding the video sink,
  whose GPU‑zero‑copy mode must match the decoder (a hardware‑mode sink fed by the software
  decoder stutters through failing GL uploads) — and both mappings are editable.
